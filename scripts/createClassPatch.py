...
source_dirs = {
    "api": str(pathlib.Path(f"{script_path}/../../hurricane-api/").resolve()),
    "server": str(pathlib.Path(f"{script_path}/../../hurricane-server/").resolve()),
}
patch_dirs = {
...
def create_patches(strings):
    for type in ("API", "Server"):
        ...
        f = open(f"0001-Hurricane-{type}-Classes.patch", "w")
